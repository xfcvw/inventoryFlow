...
function roleLabel(role) {
    const key = `role${String(role || '').charAt(0).toUpperCase()}${String(role || '').slice(1)}`;
    return t(key);
}

function planLabel(plan) {
    const map = { free: 'planFree', pro: 'planPro', business: 'planBusiness' };
    return t(map[plan] || plan);
}
...
function renderWorkspaceHeader() {
    ...
    document.getElementById('workspace-role').textContent = workspaceMeta?.role ? roleLabel(workspaceMeta.role) : '—';
    document.getElementById('workspace-plan').textContent = workspaceMeta?.plan ? planLabel(workspaceMeta.plan) : '—';
    applyRoleUI();
}
...
function renderProducts() {
    ...
    document.getElementById('products-table').innerHTML = rows.length ? rows.map(p => `
        <tr><td><strong>${esc(p.name)}</strong>${p.active ? '' : `<br><small class="muted-text">${t('inactive')}</small>`}</td><td>${esc(p.sku)}</td><td>${esc(p.category_relation?.name || p.category || '—')}</td><td>${esc(p.supplier?.name || '—')}</td><td>${money(p.price)}</td><td class="${Number(p.stock) <= Number(p.min_stock) ? 'stock-low' : ''}">${p.stock}</td><td><div class="table-actions">${can('manager') ? `<button class="small-button" data-product-edit="${p.id}">${t('edit')}</button>` : ''}${can('admin') ? `<button class="small-button danger" data-product-delete="${p.id}">${t('delete')}</button>` : ''}</div></td></tr>
    `).join('') : `<tr><td colspan="7" class="empty-state">${t('noProducts')}</td></tr>`;
}

function fillProductSelects(product = null) {
    document.getElementById('product-category').innerHTML = `<option value="">${t('uncategorized')}</option>` + categoriesCache.map(c => `<option value="${c.id}">${esc(c.name)}</option>`).join('');
    document.getElementById('product-supplier').innerHTML = `<option value="">${t('noSupplier')}</option>` + suppliersCache.map(s => `<option value="${s.id}">${esc(s.name)}</option>`).join('');
    if (product) {
...
function renderCatalogLists() {
    document.getElementById('categories-list').innerHTML = categoriesCache.length ? categoriesCache.map(c => `<div class="stack-item"><div><strong>${esc(c.name)}</strong><br><small>${c.products_count} ${t('productsWord')}</small></div><div class="table-actions">${can('manager') ? `<button class="small-button" data-category-edit="${c.id}">${t('edit')}</button>` : ''}${can('admin') ? `<button class="small-button danger" data-category-delete="${c.id}">×</button>` : ''}</div></div>`).join('') : `<div class="empty-state">${t('noCategories')}</div>`;
    document.getElementById('suppliers-list').innerHTML = suppliersCache.length ? suppliersCache.map(s => `<div class="stack-item"><div><strong>${esc(s.name)}</strong><br><small>${esc(s.email || s.contact_name || t('noContact'))} · ${s.products_count} ${t('productsWord')}</small></div><div class="table-actions">${can('manager') ? `<button class="small-button" data-supplier-edit="${s.id}">${t('edit')}</button>` : ''}${can('admin') ? `<button class="small-button danger" data-supplier-delete="${s.id}">×</button>` : ''}</div></div>`).join('') : `<div class="empty-state">${t('noSuppliers')}</div>`;
    document.getElementById('warehouses-list').innerHTML = warehousesCache.length ? warehousesCache.map(w => `<div class="stack-item"><div><strong>${esc(w.name)}</strong> ${w.is_default ? `<span class="badge badge-in">${t('defaultLabel')}</span>` : ''}<br><small>${esc(w.code)} · ${w.stocks_count} ${t('stockRows')}</small></div><div class="table-actions">${can('admin') ? `<button class="small-button" data-warehouse-edit="${w.id}">${t('edit')}</button>${w.is_default ? '' : `<button class="small-button danger" data-warehouse-delete="${w.id}">×</button>`}` : ''}</div></div>`).join('') : `<div class="empty-state">${t('noWarehouses')}</div>`;
}

function categoryModal(category = null) {
    openGenericModal({ title: category ? t('editCategory') : t('newCategory'), fields: [{ name: 'name', label: t('name'), required: true }], values: category || {}, onSubmit: async payload => { await api(category ? `/api/categories/${category.id}` : '/api/categories', { method: category ? 'PUT' : 'POST', body: JSON.stringify(payload) }); await loadCatalog(); } });
}
function supplierModal(supplier = null) {
    const fields = [{name:'name',label:t('name'),required:true},{name:'email',label:t('email'),type:'email'},{name:'phone',label:t('phone')},{name:'contact_name',label:t('contactName')},{name:'notes',label:t('notes'),type:'textarea'}];
    openGenericModal({ title: supplier ? t('editSupplier') : t('newSupplier'), fields, values: supplier || {}, onSubmit: async payload => { await api(supplier ? `/api/suppliers/${supplier.id}` : '/api/suppliers', { method: supplier ? 'PUT' : 'POST', body: JSON.stringify(payload) }); await loadCatalog(); } });
}
function warehouseModal(warehouse = null) {
    const fields = [{name:'name',label:t('name'),required:true},{name:'code',label:t('code'),required:true},{name:'is_default',label:t('defaultWarehouse'),type:'checkbox'}];
    openGenericModal({ title: warehouse ? t('editWarehouse') : t('newWarehouse'), fields, values: warehouse || {}, onSubmit: async payload => { await api(warehouse ? `/api/warehouses/${warehouse.id}` : '/api/warehouses', { method: warehouse ? 'PUT' : 'POST', body: JSON.stringify(payload) }); await loadCatalog(); } });
}
...
function renderCustomers() {
    ...
    document.getElementById('customers-table').innerHTML=rows.length?rows.map(c=>`<tr><td><strong>${esc(c.name)}</strong></td><td>${esc(c.email||'—')}</td><td>${esc(c.phone||'—')}</td><td>${c.orders_count}</td><td><div class="table-actions">${can('member')?`<button class="small-button" data-customer-edit="${c.id}">${t('edit')}</button>`:''}${can('manager')?`<button class="small-button danger" data-customer-delete="${c.id}">${t('delete')}</button>`:''}</div></td></tr>`).join(''):`<tr><td colspan="5" class="empty-state">${t('noCustomers')}</td></tr>`;
}
function customerModal(customer=null){const fields=[{name:'name',label:t('name'),required:true},{name:'email',label:t('email'),type:'email'},{name:'phone',label:t('phone')},{name:'document',label:t('document')},{name:'notes',label:t('notes'),type:'textarea'}];openGenericModal({title:customer?t('editCustomer'):t('newCustomer'),fields,values:customer||{},onSubmit:async payload=>{await api(customer?`/api/customers/${customer.id}`:'/api/customers',{method:customer?'PUT':'POST',body:JSON.stringify(payload)});await loadCustomers();}})}
...
function renderOrders(){
    ...
    document.getElementById('orders-table').innerHTML=rows.length?rows.map(o=>`<tr><td>#${o.id}</td><td>${esc(o.customer)}</td><td>${o.items?.reduce((a,i)=>a+Number(i.quantity),0)||0}</td><td>${money(o.total)}</td><td><span class="badge badge-${o.status}">${t(o.status)}</span></td><td>${when(o.created_at)}</td><td><div class="table-actions">${can('member')&&o.status==='pending'?`<button class="small-button" data-order-status="processing" data-order-id="${o.id}">${t('process')}</button>`:''}${can('member')&&o.status==='processing'?`<button class="small-button" data-order-status="completed" data-order-id="${o.id}">${t('complete')}</button>`:''}${can('member')&&!['completed','cancelled'].includes(o.status)?`<button class="small-button danger" data-order-status="cancelled" data-order-id="${o.id}">${t('cancel')}</button>`:''}</div></td></tr>`).join(''):`<tr><td colspan="7" class="empty-state">${t('noOrders')}</td></tr>`;
}

async function openOrderModal(){await ensureOrderCaches();document.getElementById('order-form').reset();document.getElementById('order-customer').innerHTML=`<option value="">${t('walkInCustomer')}</option>`+customersCache.map(c=>`<option value="${c.id}">${esc(c.name)}</option>`).join('');document.getElementById('order-warehouse').innerHTML=warehousesCache.map(w=>`<option value="${w.id}">${esc(w.name)}</option>`).join('');document.getElementById('order-items').innerHTML='';addOrderItem();document.getElementById('order-error').textContent='';document.getElementById('order-modal').classList.remove('hidden');updateOrderPreview();}
function closeOrderModal(){document.getElementById('order-modal').classList.add('hidden')}
function addOrderItem(){
    const row=document.createElement('div');row.className='order-item-row';row.innerHTML=`<select class="order-item-product" required><option value="">${t('selectProductShort')}</option>${productsCache.filter(p=>p.active).map(p=>`<option value="${p.id}" data-price="${p.price}">${esc(p.name)} · ${esc(p.sku)}</option>`).join('')}</select><input class="order-item-qty" type="number" min="1" value="1" required><input class="order-item-price" type="number" min="0" step="0.01" value="0" required><button type="button" class="small-button danger order-item-remove">×</button>`;document.getElementById('order-items').appendChild(row);row.querySelector('.order-item-product').addEventListener('change',e=>{row.querySelector('.order-item-price').value=e.target.selectedOptions[0]?.dataset.price||0;updateOrderPreview()});row.querySelectorAll('input').forEach(i=>i.addEventListener('input',updateOrderPreview));row.querySelector('.order-item-remove').addEventListener('click',()=>{row.remove();updateOrderPreview()});
}
...
async function changeOrderStatus(id,status){try{await api(`/api/orders/${id}`,{method:'PUT',body:JSON.stringify({status})});await loadOrders();productsCache=[];toast(t('orderUpdated'));loadNotifications();}catch(error){toast(error.message)}}

function prepareReports(){const now=new Date();const first=new Date(now.getFullYear(),now.getMonth(),1);document.getElementById('report-from').value=first.toISOString().slice(0,10);document.getElementById('report-to').value=now.toISOString().slice(0,10);document.getElementById('report-content').classList.add('hidden');document.getElementById('report-gate').textContent=(workspaceMeta?.limits?.reports===false)?t('reportsRequirePaid'):t('chooseReportPeriod');}
async function loadReport(){try{const from=document.getElementById('report-from').value;const to=document.getElementById('report-to').value;const data=await api(`/api/reports/overview?from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}`);document.getElementById('report-gate').textContent='';document.getElementById('report-content').classList.remove('hidden');document.getElementById('report-orders').textContent=data.sales.orders;document.getElementById('report-revenue').textContent=money(data.sales.revenue);document.getElementById('report-average').textContent=money(data.sales.average_ticket);document.getElementById('report-inventory-value').textContent=money(data.inventory_value);document.getElementById('report-top-products').innerHTML=data.top_products.length?data.top_products.map(p=>`<div class="stack-item"><div><strong>${esc(p.product_name)}</strong><br><small>${esc(p.sku||'—')}</small></div><div><strong>${p.units} ${t('units')}</strong><br><small>${money(p.revenue)}</small></div></div>`).join(''):`<div class="empty-state">${t('noSalesPeriod')}</div>`;document.getElementById('report-low-stock').innerHTML=data.low_stock.length?data.low_stock.map(r=>`<div class="stack-item"><div><strong>${esc(r.product?.name)}</strong><br><small>${esc(r.warehouse?.name)}</small></div><strong class="stock-low">${r.quantity}</strong></div>`).join(''):`<div class="empty-state">${t('noLowStock')}</div>`;}catch(error){document.getElementById('report-gate').textContent=error.message;document.getElementById('report-content').classList.add('hidden')}}

async function loadTeam(){try{const [team,invitations]=await Promise.all([api('/api/team'),api('/api/invitations')]);document.getElementById('team-table').innerHTML=team.map(m=>`<tr><td>${esc(m.name)}</td><td>${esc(m.email)}</td><td>${m.role==='owner'?`<span class="badge badge-in">${roleLabel('owner')}</span>`:`<select class="team-role-select" data-user-id="${m.id}"><option value="admin" ${m.role==='admin'?'selected':''}>${roleLabel('admin')}</option><option value="manager" ${m.role==='manager'?'selected':''}>${roleLabel('manager')}</option><option value="member" ${m.role==='member'?'selected':''}>${roleLabel('member')}</option><option value="viewer" ${m.role==='viewer'?'selected':''}>${roleLabel('viewer')}</option></select>`}</td><td>${m.role==='owner'?'—':`<button class="small-button danger" data-team-remove="${m.id}">${t('remove')}</button>`}</td></tr>`).join('');document.getElementById('invitations-list').innerHTML=invitations.length?invitations.map(i=>`<div class="stack-item"><div><strong>${esc(i.email)}</strong><br><small>${roleLabel(i.role)} · ${t('expires')} ${when(i.expires_at)}</small></div><button class="small-button danger" data-invite-delete="${i.id}">×</button></div>`).join(''):`<div class="empty-state">${t('noPendingInvitations')}</div>`;}catch(error){toast(error.message)}}
function inviteModal(){openGenericModal({title:t('inviteTeamMember'),fields:[{name:'email',label:t('email'),type:'email',required:true},{name:'role',label:t('role'),type:'select',options:['admin','manager','member','viewer'].map(v=>({value:v,label:roleLabel(v)}))}],onSubmit:async payload=>{const result=await api('/api/invitations',{method:'POST',body:JSON.stringify(payload)});await loadTeam();toast(`${t('invitationSent')} ${result.accept_url}`);}})}

async function loadBilling(){try{const data=await api('/api/billing');document.getElementById('billing-mode').innerHTML=`<strong>${t('localBillingSimulator')}</strong><br>${t('localBillingDescription')}`;document.getElementById('plans-grid').innerHTML=data.plans.map(plan=>{const label=planLabel(plan.key);return `<article class="panel plan-option ${plan.key===data.current_plan?'current-plan':''}"><span class="eyebrow">${esc(label)}</span><h2>${esc(label)}</h2><div class="usage-row"><span>${t('productsUsage')}</span><strong>${plan.products??t('unlimited')}</strong></div><div class="usage-row"><span>${t('members')}</span><strong>${plan.members??t('unlimited')}</strong></div><div class="usage-row"><span>${t('warehouses')}</span><strong>${plan.warehouses??t('unlimited')}</strong></div><div class="usage-row"><span>${t('reports')}</span><strong>${plan.reports?t('yes'):t('no')}</strong></div><button class="${plan.key===data.current_plan?'ghost-button':'primary-button'} full-button" data-change-plan="${plan.key}" ${plan.key===data.current_plan?'disabled':''}>${plan.key===data.current_plan?t('currentPlan'):t('switchPlan')}</button></article>`}).join('');}catch(error){toast(error.message)}}
async function changePlan(plan){if(!confirm(trTemplate('switchPlanConfirm',{plan:planLabel(plan)})))return;try{await api('/api/billing/change-plan',{method:'POST',body:JSON.stringify({plan})});await loadWorkspaceContext();await loadBilling();toast(t('planChanged'));}catch(error){toast(error.message)}}

async function loadAudit(){try{const logs=await api('/api/audit-logs');document.getElementById('audit-table').innerHTML=logs.length?logs.map(log=>`<tr><td>${when(log.created_at)}</td><td>${esc(log.actor?.name||t('system'))}</td><td><code>${esc(log.action)}</code></td><td>${esc((log.subject_type||'').split('\\').pop())}${log.subject_id?' #'+log.subject_id:''}</td><td><small>${esc(JSON.stringify(log.metadata||{})).slice(0,180)}</small></td></tr>`).join(''):`<tr><td colspan="5" class="empty-state">${t('noAuditEntries')}</td></tr>`;}catch(error){toast(error.message)}}

async function loadWorkspaceSettings(){try{workspaceMeta=await api('/api/workspace');renderWorkspaceHeader();document.getElementById('workspace-name').value=workspaceMeta.name;document.getElementById('workspace-slug').value=workspaceMeta.slug;document.getElementById('workspace-currency').value=workspaceMeta.currency;document.getElementById('workspace-locale').value=workspaceMeta.locale;document.getElementById('workspace-timezone').value=workspaceMeta.timezone;document.getElementById('workspace-business-type').value=workspaceMeta.business_type||'';document.getElementById('settings-plan').textContent=planLabel(workspaceMeta.plan);const limitText=(usage,limit)=>`${usage} / ${limit??'∞'}`;document.getElementById('settings-products-usage').textContent=limitText(workspaceMeta.usage.products,workspaceMeta.limits.products);document.getElementById('settings-members-usage').textContent=limitText(workspaceMeta.usage.members,workspaceMeta.limits.members);document.getElementById('settings-warehouses-usage').textContent=limitText(workspaceMeta.usage.warehouses,workspaceMeta.limits.warehouses);}catch(error){toast(error.message)}}
...
async function deleteEntity(endpoint, reload){if(!confirm(t('deleteRecordConfirm')))return;try{await api(endpoint,{method:'DELETE'});await reload();}catch(error){toast(error.message)}}
...
document.addEventListener('languageChanged', () => {
    ...
    renderWorkspaceHeader();
    const active = document.querySelector('.nav-button.active')?.dataset.section || 'dashboard';
    showSection(active);
});
